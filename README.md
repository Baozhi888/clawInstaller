...
* 📦 **环境绝对隔离**：自动下载便携版 Node.js (v20) 和 MinGit，不污染你的系统全局变量，随用随删。
* 🚀 **专为国内网络优化**：
...
1. **下载程序**：在 [Releases](#) 页面下载最新的 `OpenClawInstaller.exe`。
2. **运行程序**：
   * 选择你想要安装 OpenClaw 的目标目录。
   * *(可选)* 在“Git 代理”框中填入你信任的 GitHub 加速链接（默认已提供）。
3. **点击“一键部署”**：喝杯咖啡，等待进度条走完（工具会自动下载环境、配置网络、隔离安装依赖）。
4. **启动 OpenClaw**：部署完成后，进入安装目录，双击运行的 `start.ps1`，即可体验你的专属 AI 网关！
